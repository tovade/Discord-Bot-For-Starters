...
module.exports = {
  ...
  run: async (client, message, args) => {
        ...
        if(!target)
        return message.channel.send("You need to specify a user id.");
        if(isNaN(target))
        return message.channel.send("This is not a user id...");
    ...
    client.channels.cache.get(channel).send(unBanEmbed)          
    }
}
